...
function ConvertDDToDMS(dd) {
    ...
    let sec = (frac * 3600 - min * 60) | 0;
    return deg + "." + min + sec;
}
function ConvertDMSToDD(dms) {
    let deg = dms | 0; // truncate dms to get deg
    let frac = Math.abs(dms - deg) * 100; // get fractional part
    let min = frac | 0; //truncate frac to get min
    frac = Math.abs(frac - min) * 100; // get fractional part
    let sec = frac | 0; //truncate frac to get sec
